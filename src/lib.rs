...
impl Lattice {
    ...
    pub fn new(width: usize, height: usize, temperature: f64, h_z: f64) -> Self {
        ...
        let sites: Vec<i8> = (0..width * height)
    ...
    pub fn simulation_step(&mut self) {
        ...
        if energy_diff <= 0.0 || self.rng.random::<f64>() < (-energy_diff / self.temperature).exp()
        {
    ...
    fn wolff_step(&mut self) {
        ...
        while let Some(current) = stack.pop() {
            for neighbor in self.get_nearest_neighbors(current) {
                if !visited[neighbor]
                    && self.sites[neighbor] == spin
                    && self.rng.random::<f64>() < p_add
                {
    ...
    fn average_magnetization(&self) -> f64 {
        self.get_total_magnetization() / (self.width * self.height) as f64
    }
